<div class="max-w-3xl mx-auto px-4 py-12">
    ...
    <div class="space-y-10">
        ...
        @error('api_error')
            <div class="p-4 bg-red-50 text-red-700 text-sm font-semibold rounded-2xl border border-red-100">
                {{ $message }}
            </div>
        @enderror
                    ...
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-2">
                            <div class="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center text-gray-600 font-bold text-xs uppercase">
                                {{ substr($comment['display_name'] ?? 'A', 0, 1) }}
                            </div>
                            <span class="text-sm font-bold text-gray-800">{{ $comment['display_name'] ?? 'Anonim' }}</span>
                        </div>
            ...
            @if(session()->has('user_token'))
                ...
                <form wire:submit.prevent="saveComment" class="space-y-4">
                    <div class="mb-2">
                        ...
                        <span class="text-sm font-bold text-gray-800">👋 {{ session('user_data.name') ?? session('user_data')['name'] ?? 'Kullanıcı' }}</span>
                    </div>
